...
public class GameplayController implements Initializable {
    ...
    //perfectly useless comment on a perfectly self explanatory variable
    ...
    private void aiMovement() {
        ...
        if(currentBallX > 200) {
            if((currentAiPaddleY + (aiPaddleHeight/2)) < currentBallY) {
                moveAiPaddleDown();
            ...
            }else if((currentAiPaddleY + (aiPaddleHeight/2)) > currentBallY) {
                moveAiPaddleUp();
                if(currentAiPaddleY <= 0) {
                    moveAiPaddleDown();
                }else if(currentAiPaddleY >= (600-aiPaddle.getHeight())) {
    ...
    private void changeLevel() throws FileNotFoundException {
        ...
        //as well as preventing the media player from updating every frame
        //it also checks if the text that shows the current level is visible, if it is then it fades it out
        if (Integer.parseInt(playerScore.getText()) == 0 && Integer.parseInt(aiScore.getText()) == 0 &&
    ...
    //writes current player score, ai score, and level to a txt file to be loaded from main menu
    @FXML
